...
check_disk() {
  >$LOCKfile
  for i in $(fdisk -l | grep -oE 'Disk /dev/x?[sv]d[b-z]' | awk '{print $2}'); do
  ...
  if [ "X$DISK_LIST" == "X" ]; then
    ...
    [ $count -gt 1 ] && {
      echo "${CWARNING}This system has at least two free disk, You must manually mount it${CEND}"
      exit 0
    }
  fi
}
...
main() {
  for i in $(echo $DISK_LIST); do
    ...
    [ -n "$(mount | grep ${i}1)" ] && {
      echo "${CFAILURE}The ${i}1 already mount${CEND}"
      echo
      exit 0
    }
